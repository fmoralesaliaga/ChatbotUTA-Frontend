...
import EditDepartmentModal from '../../components/Modals/EditDepartmentModal'; // Importa el componente de modal de edición
...
export default function DeparmentsPage() {
  ...
  const openEditModal = (department) => {
    setEditModalOpen(department.id);
  };
  ...
  const deleteDepartment = async (departmentId) => {
    try {
      ...
      // Llama a la función fetchUsers para actualizar la lista de usuarios después de la eliminación
      fetchDepartments();
    } catch (error) {
  ...
  const handleCreateDepartment = async (formData) => {
    try {
      ...
      fetchDepartments(); // Actualiza la lista de departamentos después de crear uno nuevo
    } catch (error) {
  ...
  return (
    ...
    <div className="relative isolate px-6 pt-14 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl py-32 sm:py-48 lg:py-56">
            <div className="text-center">
                <ul role="list" className="divide-y divide-gray-100">
                    ...
                    <li key={department.id} className="flex justify-between gap-x-6 py-5">
                        <div className="flex min-w-0 gap-x-4">
                            <div className="min-w-0 flex-auto">
                                <p className="text-sm font-semibold leading-6 text-gray-900">{department.name}</p>
                            </div>
                        </div>
                        <div className="flex min-w-0 gap-x-4">
                        ...
                        <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                            <div className="flex gap-x-6"> 
                                <button className="-mx-1 block rounded-lg bg-emerald-600 px-5 py-2.5 text-base font-semibold text-white leading-7 text-gray-900 hover:bg-emerald-700" onClick={() => openEditModal(department)}>
                                    Editar
                                </button>
                                {editModalOpen === department.id && <EditDepartmentModal department={department} isOpen={true} onClose={closeEditModal} fetchDepartments={fetchDepartments}/>}
                                <button className="-mx-1 block rounded-lg bg-red-500 px-5 py-2.5 text-base font-semibold text-white leading-7 text-gray-900 hover:bg-red-600" onClick={() => deleteDepartment(department.id)}>
                                    Eliminar
                                </button>
                            </div>
                        </div>
        ...
        {isCreateModalOpen && (<CreateDepartmentModal isOpen={isCreateModalOpen} onClose={closeCreateModal} onCreate={handleCreateDepartment}/>
      )}
        <div
